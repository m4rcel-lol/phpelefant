...
def render_quote_card(data: QuoteCardData) -> bytes:
    ...
    meta_font = load_font(38)
    header_font = load_font(38, bold=False)

    avatar_size = 320
    
    col_left_min_h = CONTENT_PAD + avatar_size + 240
    ...
    header_text_y = inner_y0 + (HEADER_H - 44) // 2
    
    ts_str = "Posted: Thu Jan 01, 1970 12:00 am"
    if data.timestamp:
        ts_str = data.timestamp.strftime("Posted: %a %b %d, %Y %I:%M %p").lower()
    
    draw.text((inner_x0 + 25, header_text_y), "■ Post subject: Re: Thoughts?", font=header_font, fill=HEADER_TEXT)
    
    time_w = text_width(draw, ts_str, header_font)
    draw.text((inner_x1 - 25 - time_w, header_text_y), ts_str, font=header_font, fill=HEADER_TEXT)
