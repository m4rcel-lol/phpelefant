...
class Fun(commands.Cog):
    ...
    async def joke(self, ctx: commands.Context) -> None:
        await ctx.send(code_block(random.choice(JOKES)))

    @commands.hybrid_command(name="meme")
    async def meme(self, ctx: commands.Context) -> None:
        await ctx.send("https://i.imgflip.com/1bij.jpg")

    @commands.hybrid_command(name="quote")
    async def quote(self, ctx: commands.Context) -> None:
        await ctx.send(code_block(random.choice(QUOTES)))

    @commands.hybrid_command(name="fact")
    async def fact(self, ctx: commands.Context) -> None:
        await ctx.send(code_block(random.choice(FACTS)))

    @commands.hybrid_command(name="8ball")
    async def eight_ball(self, ctx: commands.Context, *, question: str = "") -> None:
        await ctx.send(code_block(random.choice(["Yes.", "No.", "Ask again later.", "It is likely.", "Do not count on it."])))

    @commands.hybrid_command(name="coinflip")
    async def coinflip(self, ctx: commands.Context) -> None:
        await ctx.send(code_block(random.choice(["Heads", "Tails"])))

    @commands.hybrid_command(name="dice")
    async def dice(self, ctx: commands.Context) -> None:
        await ctx.send(code_block(f"Rolled {random.randint(1, 6)}"))

    @commands.hybrid_command(name="roll")
    async def roll(self, ctx: commands.Context, sides: int = 6) -> None:
        if not 2 <= sides <= 100000:
            await ctx.send(code_block("Sides must be between 2 and 100000."))
            return
        await ctx.send(code_block(f"Rolled {random.randint(1, sides)} on a d{sides}."))

    @commands.hybrid_command(name="ship")
    async def ship(self, ctx: commands.Context, *, names: str = "PHP elefant") -> None:
        await ctx.send(code_block(f"Compatibility for {names}: {random.randint(1, 100)}%"))
    ...
    async def roast(self, ctx: commands.Context, member: discord.Member | None = None) -> None:
        ...
        await ctx.send(code_block(f"{target}, your code has more TODOs than a Monday standup."))
    ...
    async def compliment(self, ctx: commands.Context, member: discord.Member | None = None) -> None:
        target = member.display_name if member else ctx.author.display_name
        await ctx.send(code_block(f"{target} is reliable, sharp, and helpful."))
    ...
    async def hug(self, ctx: commands.Context, member: discord.Member | None = None) -> None:
        ...
        await ctx.send(code_block(f"Hug sent to {target}."))
    ...
    async def slap(self, ctx: commands.Context, member: discord.Member | None = None) -> None:
        ...
        await ctx.send(code_block(f"A harmless slap was delivered to {target}."))
    ...
    async def dog(self, ctx: commands.Context) -> None:
        await ctx.send("https://placedog.net/640/480?random")
    ...
    async def quiz(self, ctx: commands.Context, question: str, answer: str) -> None:
        await ctx.send(code_block(f"Quiz: {question}\nAnswer: {answer}"))
