...
from phpelefant_discord.db.models import BadWord, GuildSettings, WhitelistedDomain, WhitelistedUser
from phpelefant_discord.utils.text import contains_invite_link, contains_link, emoji_ratio, extract_domains, mention_count, normalize_text, uppercase_ratio
...
class SpamMemory:
    ...
    def update(self, guild_id: int, user_id: int, text: str, now: datetime, window_seconds: int) -> tuple[int, int]:
...
async def is_whitelisted(session: AsyncSession, guild_id: int, user_id: int) -> bool:
    row = await session.scalar(select(WhitelistedUser).where(WhitelistedUser.guild_id == guild_id, WhitelistedUser.user_id == user_id))
    return row is not None


async def bad_words(session: AsyncSession, guild_id: int) -> list[str]:
    result = await session.scalars(select(BadWord.word).where(BadWord.guild_id == guild_id))
    return [word.casefold() for word in result]


async def whitelisted_domains(session: AsyncSession, guild_id: int) -> set[str]:
    result = await session.scalars(select(WhitelistedDomain.domain).where(WhitelistedDomain.guild_id == guild_id))
...
def analyze_message(
    *,
    ...
    untrusted = {domain for domain in domains if domain not in trusted_domains}
    ...
    if settings.anti_link_enabled and contains_invite_link(text):
        return SpamDecision(AutoAction.TIMEOUT, "invite link")
    if settings.anti_link_enabled and contains_link(text) and untrusted:
    ...
    if settings.anti_spam_enabled and flood_count > settings.flood_max_messages:
        return SpamDecision(AutoAction.TIMEOUT, "message flooding")
