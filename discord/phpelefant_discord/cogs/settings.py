...
class SettingsCog(commands.Cog, name="Settings"):
    ...
    async def set_bool(self, ctx: commands.Context, attr: str, value: str, label: str) -> None:
        ...
        if enabled is None:
            await ctx.send(embed=code_embed("Settings", f"Use {label} on or {label} off."))
            return
        ...
        await ctx.send(embed=code_embed("Settings", f"{label} set to {enabled}."))
    ...
    async def setlogchannel(self, ctx: commands.Context, channel: discord.TextChannel | None = None) -> None:
        ...
        await ctx.send(embed=code_embed("Settings", f"Log channel set to {channel.mention if channel else 'off'}."))
    ...
    async def setwarnlimit(self, ctx: commands.Context, limit: int) -> None:
        ...
        await ctx.send(embed=code_embed("Settings", f"Warning limit set to {limit}."))
    ...
    async def badwords(self, ctx: commands.Context) -> None:
        async with session_scope(self.bot.session_factory) as session:
            ...
            words = list(result)
        await ctx.send(embed=code_embed("Bad Words", ", ".join(words) if words else "No bad words configured."))
    ...
    async def badwords_add(self, ctx: commands.Context, *, word: str) -> None:
        ...
        await ctx.send(embed=code_embed("Bad Words", "Bad word added."))
    ...
    async def badwords_remove(self, ctx: commands.Context, *, word: str) -> None:
        ...
        await ctx.send(embed=code_embed("Bad Words", "Bad word removed if present."))
    ...
    async def whitelist(self, ctx: commands.Context) -> None:
        ...
        await ctx.send(embed=code_embed("Whitelist", body))
    ...
    async def whitelist_add(self, ctx: commands.Context, member: discord.Member, *, reason: str = "trusted") -> None:
        ...
        await ctx.send(embed=code_embed("Whitelist", f"Whitelisted {member.id}."))
    ...
    async def whitelist_remove(self, ctx: commands.Context, member: discord.Member) -> None:
        ...
        await ctx.send(embed=code_embed("Whitelist", f"Removed {member.id} if present."))
    ...
    async def whitelist_domain(self, ctx: commands.Context, domain: str) -> None:
        ...
        await ctx.send(embed=code_embed("Whitelist", f"Whitelisted domain {domain}."))
